...
signedMintingPolicy :: POSIXTime -> PaymentPubKeyHash -> () -> ScriptContext -> Bool
signedMintingPolicy ptime pkh _ sContext = traceIfFalse "No proper signature" ifSigned &&
                                           traceIfFalse "Can't no longer mint" deadlineNotReached 
   ...
   where
    ...
    deadlineNotReached = True 
      -- <Homework> (condition of been able to mint BEFORE deadline is reached)

signedpolicy :: POSIXTime -> PaymentPubKeyHash -> Scripts.MintingPolicy
signedpolicy pkh = mkMintingPolicyScript $ 
                                   $$(PlutusTx.compile [|| Scripts.wrapMintingPolicy . signedMintingPolicy ||])
                                   `PlutusTx.applyCode`
                                   PlutusTx.liftCode pkh 
                                   --Optional: Fix this code so it works!
